    ...
    WriteContext *wctx)
{
  dout(10) << __func__ << " 0x" << std::hex << offset << "~" << length
	   << std::dec << dendl;
  ceph_assert(length < min_alloc_size); //断言确保数据的长度< alloc_size大小，系统默认hdd为64KB
  ...
  auto max_bsize = std::max(wctx->target_blob_size, min_alloc_size); //max_bsize大小是512KB
  auto min_off = offset >= max_bsize ? offset - max_bsize : 0;   //一般都为0
  uint32_t alloc_len = min_alloc_size;   //alloc_len: 64KB
  ...
  offset : 0
  max_bsize: 512KB
  o->extent_map.fault_range(db, 0, 512KB);
  */
  ...
  // into 2 max blobs
  size_t blob_threshold = max_blob_size / min_alloc_size * 2 + 1;  #max_blob_size:512KB min_alloc_size:64KB
  bool above_blob_threshold = false;

  inspected_blobs.reserve(blob_threshold);  //定义了一个结构体flat_set inspected_blobs，大小是blob_threshold这么多个blobs
    ...
    if (ep != end && ep->logical_offset < offset + max_bsize) {
      ...
      if (bstart >= end_offs) {
        ...
        */
        ...
        uint64_t head_read = p2phase(b_off, chunk_size);  //按照chunk size对齐off写入偏移
        uint64_t tail_read = p2nphase(b_off + b_len, chunk_size); //按照chunk size对齐len写入大小
        if ((head_read || tail_read) &&
            (b->get_blob().get_ondisk_length() >= b_off + b_len + tail_read) &&
            head_read + tail_read < min_alloc_size) {
          b_off -= head_read;              //b_off应该是实际的写入偏移
          b_len += head_read + tail_read;  //b_len应该是实际的写入长度

        } else {
          head_read = tail_read = 0;  //否则都是0 
        }
	        ...
	        b->get_blob().is_allocated(b_off, b_len)) 
        {
          ...
          //bluestore_debug_omit_block_device_write默认是false
          if (!g_conf()->bluestore_debug_omit_block_device_write) {
            bluestore_deferred_op_t *op = _get_deferred_op(txc);
            op->op = bluestore_deferred_op_t::OP_WRITE;
  ...
  WriteContext *wctx)
{
  ...
  for (auto& wi : wctx->writes) {
    if (c && wi.blob_length > min_alloc_size) {
      ...
      log_latency("compress@_do_alloc_write", l_bluestore_compress_lat, mono_clock::now() - start,
	      cct->_conf->bluestore_log_op_age );
    } else {
      need += wi.blob_length;
    }
